...
class ClusterAlgorithm:
    ...
    def go_cluster(self, dlimit):
        ...
        while( merge ):
            ...
            self.clusters = list(filter( lambda c: not c is None, self.clusters ))
    ...
    def draw_matrix(self, fig_size):
        ...
        plot_data = np.array([[0.0] * max_cols for r in range( max_cols )])
        plot_xx = np.array([[int(r * scale) for c in range( max_cols )] for r in range( max_cols )])
        plot_yy = np.array([[int(c * scale) for c in range( max_cols )] for r in range( max_cols )])
